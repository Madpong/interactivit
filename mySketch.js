...
function draw() {
  ...
  for (let i = 0; i < num; i++) {
	...
	angle += speed;
	//if(value){
	fill( 45 , sin(angle)*200 , 255);
	//fill( sin(angle)*50, sin(angle)*255 , 0);
	//let disText = textWidth(tittle);
	push();
	translate(-4,4);
	noFill();
	textAni();
...
function circlePhase(x, y, phase,d){
	...
	stroke(285, 50+(angleColor*255) , 91 +(angleColor*255), 153)
  ...
  if (value === true) {
	...
	function textAni(){
			...
			for(let i = 0; i < tittle.length; i ++){
				...
				let distance = abs(totalOffset - (mouseX-width/3)-100);
				...
				let letterWidth = textWidth(tittle.charAt(i))+7.0;
